...
import './CorporateMattersPage.scss';
...
export default React.memo((props) => {
    ...
    const tableColumns = [
        {
            ...
            field: 'id',
            render: rowData => <Link
                to={`${buildInstanceURL()}/corporate-matter/${rowData?.id}`}
    ...
    const getFilter = () => {
        return {
            category: {
                value: LEGAL_CASES_CATEGORIES.corporateMatters
            }
        }
    }
    ...
    const exportData = () => {
        ...
        LegalCase.exportGrid({
            ...
            fileDownload(response.data, "Exported Corporate Matters.xls");
        }).catch((error) => {
    ...
    return (
        <APPageContainer
            id={PAGES_IDS.corporateMattersPage}
        >
            <APPageHeader>
                <APPageTitle
                    pageTitle={PAGES_TITLES.corporateMattersPage}
                />
            </APPageHeader>
            <APPageBody>
                <APMaterialTable
