...
export default React.memo((props) => {
        ...
        CustomField.getList(
            {
                ...
                "caseType": {
                    "value": props?.corporateMatter?.corporateMatter?.case_type_id
                }
            }, 
            CustomField.allRelations,
            JSON.stringify([{"relation": "customFieldValues", "filters": [{"name": "recordId", "value": props?.corporateMatter?.id}]}])
        ).then((response) => {

            setCustomFields(response?.data?.data);
        }).catch((error) => {

            console.log('loading corporate matter custom fields', error);
        }).finally(() => {
    ...
    const getCustomFieldValue = (customField) => {
        ...
        switch (customField?.type) {
            ...
            case "date_time":
                let dateValue = customField?.custom_field_values?.[0]?.date_value ?? null;
                let timeValue = customField?.custom_field_values?.[0]?.time_value ?? null;
            ...
            default:
                value = customField?.custom_field_values?.[0]?.text_value ?? null;
                break;
        }
    ...
    for (var i = 0; i < customFields.length;) {
        ...
        let record = <Grid
            container
            className="field-row"
            key={"corporate-matter-page-general-info-panel-custom-fields-field-row-" + i}
        >
            <React.Fragment>
                <Grid
                    item
    ...
    return (
        <Container
            id="corporate-matter-page-general-info-panel-custom-fields"
            maxWidth={false}
            className="section"
        >
            {rows}
