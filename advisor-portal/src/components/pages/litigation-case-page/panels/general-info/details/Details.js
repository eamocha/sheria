...
export default React.memo((props) => {
    ...
    const [formData, setFormData] = useState({
        assignees: props?.litigationCase?.outsources_by_advisor ? props?.litigationCase?.outsources_by_advisor[0]?.outsource_contacts.map((item, index) => {
    ...
    const save = () => {
        ...
        LegalCase.updateOutsourceingContacs(props?.litigationCase?.outsources_by_advisor[0]?.id, submitData).then((response) => {
            globalStateDispatcher({
            ...
            globalStateDispatcher({
                notificationBar: {
                    ...globalState?.notificationBar,
                    open: true,
                    text: "Ligalcase has been updated successfully",
                    severity: "success"
                }
            });
    ...
    const items = [
        ...
        },
        {
            ...
            value: props?.litigationCase?.outsourced_to,
            fullWidth: true,
            inline_edit:
                <React.Fragment>
                    <Grid
    ...
    for (var i = 0; i < items.length;) {
        ...
        let record = <Grid
            container
            className="field-row"
            key={"litigation-case-page-general-info-panel-details-field-row-" + i}
        >
            {
                row?.fullWidth ?
                    (
    ...
    return (
        <Container
            id="litigation-case-page-general-info-panel-details"
            maxWidth={false}
            className="section"
        >
            {rows}
