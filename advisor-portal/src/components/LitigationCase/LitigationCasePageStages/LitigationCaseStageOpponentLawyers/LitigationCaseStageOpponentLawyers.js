...
import './LitigationCaseStageOpponentLawyers.scss';
import {
    Container,
    Button,
    Collapse,
...
export default React.memo((props) => {
    const [isExpanded, setIsExpanded] = useState(props?.stageOpponentLawyers?.length > 0);
    ...
    let rows = stageOpponentLawyers.map((item, key) => {
        return (
            <TableRow key={key}>
                ...
                <TableCell>
                    <IconButton
                    // onClick={(event) => this.props.removeItem(event, key)}
                    >
                        <DeleteIcon />
                    </IconButton>
                </TableCell>
    ...
    return (
        <React.Fragment>
            ...
            >
                ...
                >
                    {isExpanded ? <ExpandLess /> : <ExpandMore />} {t("opponent_lawyer")}
                </Button>
                <IconButton
                    onClick={() => openForm()}
                >
