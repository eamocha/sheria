/**
 * This form is like create stage.
 * it used when the user wants to move from a stage to another, so actually he is creating a new litigation case stage record (legal case litigation details)
 */
import React, {
    useContext,
    useEffect,
...
export default React.memo((props) => {
    const formId = FORMS_NAMES.litigationCaseStageChangeForm;
    ...
    const loadStagesList = (data) => {
        /**
         * remove the current stage from the list
         * so the advisor can't change the stage to itself!!
         */
        let modifiedData = data.filter((item) => {

            return item?.id != globalState?.modal?.form?.data?.litigationCase?.current_stage?.stage_name?.id
        });

        let result = loadListWithLanguages(modifiedData, 'stage_languages', 'name', 'id');
    ...
    const submit = (e) => {
        ...
        LitigationCaseStage.create(data).then((response) => {
            ...
            globalStateDispatcher({
                notificationBar: {
                    ...
                    open: true,
                    severity: "success",
                    text: "Stage has been changed successfully"
                }
            });
        }).catch((error) => {
            let message = error?.response?.data?.message;
