...
function contractGridInitialization() {
    gridDataSrc = new kendo.data.DataSource({
        transport: {
            read: {
                url: getBaseURL() + "cases/related_contracts/" + jQuery('#caseIdFilter', '#contractSearchFilters').val(),
                dataType: "JSON",
                type: "POST",
                complete: function () {
    ...
    gridOptions = {
        ...
        columns: [
            {
                field: "",
                template: '<div class="wraper-actions"><div class="list-of-actions"><a class="pull-right p-0" href="javascript:;" onclick="relatedContractDelete(#= contract_id #)"><i class="fa fa-fw fa-trash"></i></a></div></div>',
                sortable: false,
                title: "",
                width: '30px'
            },
...
function relateContractAdd() {
    var caseId = jQuery('#caseIdFilter', '#contractSearchFilters').val();
    var newRelatedContract = jQuery('#lookup-contract-id', '#related-contract-form').val();
    jQuery.ajax({
        url: getBaseURL() + 'cases/related_contract_add',
        dataType: "json",
        type: "POST",
        data: {legal_case_id: caseId, related_contract_id: newRelatedContract},
...
function relatedContractDelete(contractcaseId) {
    confirmationDialog('confim_delete_action', {resultHandler: _relatedContractDelete, parm: contractcaseId});
}

function _relatedContractDelete(id) {
    var caseId = jQuery('#caseIdFilter', '#contractSearchFilters').val();
    jQuery.ajax({
        url: getBaseURL() + 'cases/related_contract_delete',
        dataType: "json",
        type: "POST",
        data: {
            caseId: caseId,
