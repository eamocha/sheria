...
function gridInitialization() {
    ...
    if (jQuery('#display-columns').val()) {
        ...
        tableColumns.push({field: 'id', title: ' ', filterable: false, sortable: false, template: '<input type="checkbox" name="caseIds[]" title="'+_lang.archiveCheckboxTitle+'" id="caseId_#= id #" value="#= id #" onchange="enableDisable_tooltips(this);" />' +
                    ...
                    '<a class="dropdown-item"  href="' + getBaseURL() + 'criminal_cases/export_to_word/#= id #">' + _lang.exportToWord + '</a>' +
        ...
        jQuery.each(columnsArray, function (i, item) {
            if (item === 'id') {
                ...
                array_push = {field: item, format: "{0:yyyy-MM-dd}", title: _lang.dateOfComplaint, width: '140px', template: "#= (caseArrivalDate == null) ? '' : kendo.toString(caseArrivalDate, 'yyyy-MM-dd') #"};
            } else if (item === 'subject') {
                array_push = {field: item, title: _lang.complaintSubject, template: '<a href="' + getBaseURL() + 'cases/complaint_details/#= id #"><bdi>#=  replaceHtmlCharacter(addslashes(subject)).replace("\\\\\\\'", "\\\'") #</bdi></a><i class="iconLegal iconPrivacy#=private#"></i>', width: '220px'};
                ...
                array_push = {field: item, title: _lang.remarks, template: '#= (statusComments!=null&&statusComments!="") ? statusComments.substring(0,40)+"..." : ""#', width: '320px'};
            }
                ...
                array_push = {field: item, format: "{0:yyyy-MM-dd}", title: _lang.dateOfApproval, width: '158px', template: "#= (dueDate == null) ? '' : kendo.toString(dueDate, 'yyyy-MM-dd') #"};
            }
           /* else if (item === 'caseClientPosition') {
                ...
                array_push = {encoded: false, sortable: !sqlsrv2008, headerAttributes: {"col-header-placeholder": sqlsrv2008 ? _lang.columnDisabledSorting : ''}, field: item, title: _lang.suspects, width: '192px', template: "#= (opponentNames == null) ? '' : opponentNames #"};
            }
                ...
                array_push = {field: item, title: _lang.natureOfCase, width: '170px'};
            }
            else if (item === 'clientName') {
                array_push = {field: item, sortable: !sqlsrv2008, headerAttributes: {"col-header-placeholder": sqlsrv2008 ? _lang.columnDisabledSorting : ''}, title: _lang.complainant, width: '175px'};
            }
            else if (item === 'status') {
