...
function contractGridInitialization() {
    gridDataSrc = new kendo.data.DataSource({
        transport: {
            read: {
                url: getBaseURL() + "contacts/related_contracts/" + jQuery('#contactIdFilter', '#contractSearchFilters').val(),
                dataType: "JSON",
                type: "POST",
                complete: function () {
    ...
    gridOptions = {
        ...
        toolbar: [{
            name: "quick-search",
            template: '<div class="col-md-4 mb-10"><div class="input-group col-md-11"><input type="text" class="form-control search margin-top quick-search-filter" placeholder=" ' + _lang.searchContract + '" name="contractLookUp" id="contractLookUp" onkeyup="contractQuickSearch(event.keyCode, this.value);" title="' + _lang.searchContract + '" /></div></div>'
                + '</div>'
        }],
    };
}
...
function relateContractAdd() {
    var contactId = jQuery('#contactIdFilter', '#contractSearchFilters').val();
    var newRelatedContract = jQuery('#lookup-contract-id', '#related-contract-form').val();
    jQuery.ajax({
        url: getBaseURL() + 'contacts/related_contract_add',
        dataType: "json",
        type: "POST",
        data: {contact_id: contactId, related_contract_id: newRelatedContract},
...
function _relatedContractDelete(id) {
    var contactId = jQuery('#contactIdFilter', '#contractSearchFilters').val();
    jQuery.ajax({
        url: getBaseURL() + 'contacts/related_contract_delete',
        dataType: "json",
        type: "POST",
        data: {
            contactId: contactId,
