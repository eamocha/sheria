jQuery(document).ready(function () {
    ...
    jQuery("#tabs-li li", '#task-workflows-management').removeClass("ui-corner-top").addClass("ui-corner-left col-md-2");
...
function workflowForm(id) {
    id = id || false;
    jQuery.ajax({
        url: getBaseURL() + 'task_workflows/' + (!id ? 'add_workflow' : ('edit_workflow/' + id)),
        type: "GET",
        dataType: "JSON",
        beforeSend: function () {
...
function statusesMigrationSubmit(container) {
    ...
    jQuery.ajax({
        url: getBaseURL() + 'task_workflows/migrate_statuses/',
        type: "POST",
        data: formData,
        dataType: "JSON",
        beforeSend: function () {
...
function workflowStatusForm(id) {
    ...
    jQuery.ajax({
        url: getBaseURL() + 'task_workflows/add_workflow_status/' + id,
        type: "POST",
        dataType: "JSON",
        beforeSend: function () {
            jQuery("#loader-global").show();
...
function workflowStatusFormSubmit(container) {
    ...
    jQuery.ajax({
        url: getBaseURL() + 'task_workflows/add_workflow_status/',
        type: "POST",
        data: formData,
        dataType: "JSON",
        beforeSend: function () {
        ...
        success: function (response) {
            if (!response.result) {
                ...
                window.location = getBaseURL() + 'task_workflows/index/' + response.workflow_id;
            }
        }, complete: function () {
            jQuery('.modal-save-btn', container).removeAttr('disabled');
            jQuery('.loader-submit', container).removeClass('loading');
...
function statusTransitionsViewForm(statusId, workflowId) {
    jQuery.ajax({
        url: getBaseURL() + 'task_workflows/view_status_transitions/' + statusId + '/' + workflowId,
        dataType: 'JSON',
        type: 'POST',
        beforeSend: function () {
            jQuery("#loader-global").show();
...
function setAsStartPoint(workflowId, statusId) {
    jQuery.ajax({
        url: getBaseURL() + 'task_workflows/set_as_start_point/' + workflowId + '/' + statusId,
        dataType: 'JSON',
        type: 'POST',
        beforeSend: function () {
            jQuery("#loader-global").show();
...
function deleteWorkflowStatus(statusId) {
    ...
    jQuery.ajax({
        url: getBaseURL() + 'task_workflows/delete_workflow_status/' + statusId + '/' + workflowId,
        type: 'POST',
        success: function (response) {
...
function deleteTransition(transitionId) {
    ...
    jQuery.ajax({
        url: getBaseURL() + 'task_workflows/delete_transition/' + transitionId,
        dataType: 'JSON',
        type: 'POST',
        beforeSend: function () {
            jQuery("#loader-global").show();
...
function deleteWorkflow(workflowId) {
    window.location = getBaseURL() + 'task_workflows/delete_workflow/' + workflowId;
}