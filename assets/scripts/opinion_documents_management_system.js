...
function moneyDownloadFile(id) {
    var downloadUrl = getBaseURL() + $documentConfig.moduleController + '/' + $documentConfig.objName + '_download_file/' + id;
...
function moneyPreviewFile(id, module, moduleId, lineage, extension) {
    jQuery.ajax({
        url: getBaseURL()  + 'vouchers/preview_document' ,
        method: 'post',
        data: {id: id},
        dataType: 'json',
        beforeSend: function () {
...
function deleteDocument(id) {
    if (confirm(_lang.confirmationDeleteFile)) {
        jQuery.ajax({
            url: getBaseURL() + $documentConfig.moduleController + '/' + $documentConfig.objName + '_delete_document',
            type: 'POST',
            dataType: 'JSON',
            data: {
                'document_id': id,
...
function renameDocument(id, name, fullName, type) {
    ...
    if (null !== name && "" !== name) {
        ...
        jQuery.ajax({
            url: getBaseURL() + $documentConfig.moduleController + '/' + $documentConfig.objName + '_' + actionName,
            type: 'POST',
            dataType: 'JSON',
            data: {
                'document_id': id,
...
function drag_and_drop() {
    ...
    $dropZone = new Dropzone('#dropbox', {
        url: getBaseURL() + $documentConfig.moduleController + '/' + $documentConfig.objName + '_upload_file',
        paramName: 'uploadDoc',
        addRemoveLinks: true,
        parallelUploads: 1,
...
jQuery(document).ready(function () {
    ...
    attachmentDocumentDataSrc = new kendo.data.DataSource({
        transport: {
            read: {
                url: getBaseURL() + $documentConfig.moduleController + '/' + $documentConfig.objName + "_load_documents",
                dataType: "JSON",
                type: "GET",
            ...
            update: {
                url: getBaseURL() + $documentConfig.moduleController + '/' + $documentConfig.objName + "_edit_documents",
                dataType: "jsonp",
                type: "POST",
                complete: function (XHRObj) {
                    $attachmentDocumentGrid.data('kendoGrid').dataSource.read();
    ...
    attachmentDocumentGridOptions = {
        ...
        columns: [
            {
                ...
                template: '<div class="dropdown">' + gridActionIconHTML +
                        ...
                        '<a class="dropdown-item #= (type == \'file\' && extension.toLowerCase() == \'pdf\')? \'\' : \'hide\' #" data-title="#= full_name #" target=\'_blank\' href="#= getBaseURL() + $documentConfig.moduleController + \'/view_document/\' + id + \'/\' + encodeURIComponent(rawurlencode(full_name)) #">' + _lang.view + '</a>' + 
            ...
            {
                field: "full_name",
                template: '<a href="javascript:;" onclick="#= type == \'folder\' # ? loadDirectoryContent(#= id #) : DownloadFile(#= id #);return false;" >#= full_name #</a>',
        ...
        toolbar: [{
                ...
                template: '<div class="col-md-3">'
                        ...
                        + '<a class="dropdown-item" href="javascript:;" onclick="uploadFileForm();">' + _lang.uploadFile + '</a>'
                        + '</div>'
                        + '</div>'
                        + '</div>'
