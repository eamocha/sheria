...
function convertToLitigation($caseId) {
    if (confirm(_lang.confirmationConvertCaseToLitigation)) {
        jQuery.ajax({
            url: getBaseURL() + 'cases/convert_case_to_prosecution/' + $caseId,
            type: 'POST',
            dataType: 'JSON',
            success: function (response) {
                var ty = 'error';
...
function gridInitialization() {
    ...
    if (jQuery('#display-columns').val()) {
        ...
        tableColumns.push({field: 'id', title: ' ', filterable: false, sortable: false, template: '<input type="checkbox" name="caseIds[]" title="'+_lang.archiveCheckboxTitle+'" id="caseId_#= id #" value="#= id #" onchange="enableDisable_tooltips(this);" />' +
                    ...
                    '<a class="dropdown-item" href="' + getBaseURL() + 'cases/investigation_case_details/#= id #">' + _lang.viewEdit + '</a>' +
                    '<a class="dropdown-item" href="' + getBaseURL() + 'cases/convert_to_prosecution/#= id #">' + _lang.convertToProsecution + '</a>' +
        ...
        jQuery.each(columnsArray, function (i, item) {
            if (item === 'id') {
                array_push = {field: item, sortable: !sqlsrv2008, headerAttributes: {"col-header-placeholder": sqlsrv2008 ? _lang.columnDisabledSorting : ''}, filterable: false, title: _lang.criminalCaseSeriaNo, template: '<a href="' + getBaseURL() + 'cases/investigation_case_details/#= id #">#= caseID #</a>', width: '120px'};
            } else if (item === 'caseArrivalDate') {
                array_push = {field: item, format: "{0:yyyy-MM-dd}", title: _lang.YearOfAction, width: '140px', template: "#= (caseArrivalDate == null) ? '' : kendo.toString(caseArrivalDate, 'yyyy-MM-dd') #"};
            } else if (item === 'subject') {
                array_push = {field: item, title: _lang.allegationComplaintDetection, template: '<a href="' + getBaseURL() + 'cases/investigation_case_details/#= id #"><bdi>#=  replaceHtmlCharacter(addslashes(subject)).replace("\\\\\\\'", "\\\'") #</bdi></a><i class="iconLegal iconPrivacy#=private#"></i>', width: '220px'};
            } else if (item === 'latest_development') {
                array_push = {field: item, sortable: !sqlsrv2008, headerAttributes: {"col-header-placeholder": sqlsrv2008 ? _lang.columnDisabledSorting : ''}, title: _lang.actionTaken, template: '#= getLatestDevelopment(latest_development) #', width: '350px'};
            } else if (item === 'assignee') {
                array_push = {field: item, sortable: !sqlsrv2008, headerAttributes: {"col-header-placeholder": sqlsrv2008 ? _lang.columnDisabledSorting : ''}, title: _lang.responsibleOfficers, template: '#= (assignee!=null && userStatus=="Inactive")? assignee+" ("+_lang.custom[userStatus]+")":((assignee!=null)?assignee:"") #', width: '192px'};
                ...
                array_push = {field: item, title: _lang.currentStatus, template: '#= (statusComments!=null&&statusComments!="") ? statusComments.substring(0,40)+"..." : ""#', width: '320px'};
            }
            else if (item === 'description') {
                array_push = {field: item, title: _lang.briefFacts, template: '#= (description!=null&&description!="") ? description.substring(0,90)+"..." : ""#', width: '320px'};
            }
                ...
                array_push = {field: item, sortable: !sqlsrv2008, headerAttributes: {"col-header-placeholder": sqlsrv2008 ? _lang.columnDisabledSorting : ''}, title: _lang.caseRiskProfile, width: '165px'};
            }
            else if (item === 'opponentNames') {
                array_push = {encoded: false, sortable: !sqlsrv2008, headerAttributes: {"col-header-placeholder": sqlsrv2008 ? _lang.columnDisabledSorting : ''}, field: item, title: _lang.nameofOperatorSuspect, width: '192px', template: "#= (opponentNames == null) ? '' : opponentNames #"};
            }
                ...
                array_push = {field: item, sortable: !sqlsrv2008, headerAttributes: {"col-header-placeholder": sqlsrv2008 ? _lang.columnDisabledSorting : ''}, title: _lang.accusedName, width: '175px'};
            }
            else if (item === 'status') {
                ...
                array_push = {field: item, sortable: !sqlsrv2008, headerAttributes: {"col-header-placeholder": sqlsrv2008 ? _lang.columnDisabledSorting : ''}, title: getTranslation("witnesses"), width: '182px'};
            }
