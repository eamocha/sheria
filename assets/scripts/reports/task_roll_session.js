jQuery(function() {
	...
	jQuery('a', '#pagination').each(function(index, element) {
		jQuery(element).click(function(e) {
			e.preventDefault();
			var containerForm = jQuery('#submitBtn2', '#taskReportQuickSearchFilters').attr('clicked') == '1' ? '#taskReportQuickSearchFilters' : '#taskReportSearchFilters';
			jQuery('#skip', containerForm).val((element.innerHTML - 1) * jQuery('#take', '#taskReportSearchFilters').val());
...
function collapseExpandRows(){
	jQuery("i.first-grouping").click(function() {
		$this = jQuery(this);
		$targetParent = $this.parent().parent().parent();
...
function changePerPage(selectedNum) {
	var containerForm = jQuery('#taskReportSearchFilters');
	jQuery('#take', containerForm).val(selectedNum.options[selectedNum.selectedIndex].value);
	jQuery('#skip', containerForm).val('0');
	containerForm.attr('action', getBaseURL() + 'reports/task_roll_session').submit();
}
function loadReportsEventsForFilters() {
	makeFieldsDatePicker({fields: [jQuery('#due_dateValue', '#taskReportSearchFilters'), jQuery('#due_dateEndValue', '#taskReportSearchFilters')]});
	userLookup(jQuery('#assigned_toValue', '#taskReportSearchFilters'), 'assigned_toValue');
	userLookup(jQuery('#reporterValue', '#taskReportSearchFilters'), 'reporterValues');
}
function functionReturnTrue() {
	return true;
}
function exportToPDF() {
    var formId = jQuery('#taskReportSearchFilters');
    jQuery('#filtersInfoForExport1', formId).val(JSON.stringify(getExportInfoFilter('taskReportSearchFilters')));
    var oldFormAction = formId.attr('action');
    formId.attr('action', getBaseURL() + 'reports/task_roll_session_pdf').submit();
    formId.attr('action', oldFormAction);
    enableAll(formId);
}
function taskReportSettings(){
    jQuery.ajax({
        url: getBaseURL() + 'reports/task_roll_session_settings',
        dataType: 'JSON',
        type: 'GET',
        beforeSend: function () {
        ...
        success: function (response) {
            if (response.html) {
                if (jQuery(".settings-dialog-container").length <= 0) {
                    ...
                    jQuery('#task-excluded-statuses').html(response.html_task_excluded_statuses);
...
function fetchCasesCheckbox(checkbox) {
    var isChecked = jQuery(checkbox).is(':checked');
    jQuery(checkbox).val(isChecked ? 'yes' : '');
