...
function changePerPage(selectedNum) {
	...
	containerForm.attr('action', getBaseURL() + 'reports/hearings_pending_updates').submit();
}
...
function exportToPDF() {
    ...
    formId.attr('action', getBaseURL() + 'reports/hearings_pending_updates_pdf').submit();
    formId.attr('action', oldFormAction);
    enableAll(formId);
}
function hearingReportSettings(){
    jQuery.ajax({
        url: getBaseURL() + 'reports/hearings_pending_updates_settings',
        dataType: 'JSON',
        type: 'GET',
        beforeSend: function () {
        ...
        success: function (response) {
            if (response.html) {
                if (jQuery(".settings-dialog-container").length <= 0) {
                    ...
                    jQuery('#hearing-business-rules').html(response.html_report_filter_criteria);
                    jQuery('.tooltip-title', settingsDialogContainer).tooltipster({
                        contentAsHTML: true,
                        timer: 22800,
                        animation: 'grow',
                        delay: 200,
