...
jQuery(document).ready(function () {
    ...
    matterAttachmentsContainer = jQuery('#matter-attachments-module', '#task-display-form');
...
function taskCommentForm(taskId, id) {
    ...
    jQuery.ajax({
        url: getBaseURL() + 'tasks/' + (!id ? 'add_comment' : 'edit_comment'),
        type: 'GET',
        data: {
            task_id: taskId,
            id: id ? id : ''
        ...
        images_upload_handler: function (blobInfo, success, failure) {
                ...
                xhr.onload = function() {
                   ...
                   if(images.includes(json.file.extension)){
                        tinymce.activeEditor.execCommand('mceInsertContent', false, '<img data-name="'+json.file.full_name+'" src="'+getBaseURL() + 'tasks/return_doc_thumbnail/'+json.file.id+'" width="100" height="100"  />');
                   }else{
                        tinymce.activeEditor.execCommand('mceInsertContent', false, '<a href="'+getBaseURL() + 'tasks/download_file/'+json.file.id+'" >'+json.file.full_name+'</a>');
                    }
...
function styleInputsComment(){
    if(tinymce.activeEditor.getContent().length > 0){
        if(jQuery("#comment-id").val()){
            jQuery("#task-comment-form #comment").val(tinymce.activeEditor.getContent());
        }
...
function commentDelete(params) {
    jQuery.ajax({
        url: getBaseURL() + 'tasks/delete_comment',
        dataType: 'json',
...
function documentDelete(id) {
    jQuery.ajax({
        ...
        success: function (response) {
            ...
            jQuery('#document-item-' + id, jQuery('#activity-module', '#task-display-form')).remove();
...
function dragAndDropMatterAttachment() {
    ...
    $dropZoneMatter = new Dropzone('#matter-attachments-module-body', {
        ...
        init: function () {
            ...
            jQuery.get(getBaseURL() + matterModuleController + '/matter_load_documents', {module: matterModule, module_record_id: matterModuleRecordId, task_record_id: moduleRecordId, lineage: matterLineage}, function (response) {
        ...
        accept: function (file, done) {
            ...
            this.options.params['task_record_id'] = moduleRecordId;
...
function reloadMatterAttachments(that, value) {
    var mockFile = {name: value.full_name, size: value.size, id: value.id, createdOn: value.display_created_on, creator: value.display_creator_full_name, extension: value.extension};
    that.emit("addedfile", mockFile);
    that.options.thumbnail.call(this, mockFile, 'tasks/return_matter_doc_thumbnail/' + value.id);
    that.emit("complete", mockFile);
    that.files.push(mockFile);
}
