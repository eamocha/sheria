jQuery(document).ready(function () {
    jQuery("#tabs", '#task-workflows-management').tabs().addClass("ui-tabs-vertical ui-helper-clearfix no-margin-left");
    jQuery("#tabs-li li", '#task-workflows-management').removeClass("ui-corner-top").addClass("ui-corner-left");
...
function workflowForm(id) {
    id = id || false;
    jQuery.ajax({
        url: getBaseURL() + 'advisor_task_workflows/' + (!id ? 'add_workflow' : ('edit_workflow/' + id)),
        type: "GET",
        dataType: "JSON",
        beforeSend: function () {
...
function workflowFormSubmit(container) {
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            ...
            console.log(response);
            if (response.result) {
                var workflowId = typeof response.advisor_task_workflow_id !== 'undefined' ? response.advisor_task_workflow_id : id;
                window.location = getBaseURL() + 'advisor_task_workflows/index/' + workflowId;
            } else {
...
function statusesMigrationSubmit(container) {
    ...
    jQuery.ajax({
        url: getBaseURL() + 'advisor_task_workflows/migrate_statuses/',
        type: "POST",
        data: formData,
        dataType: "JSON",
        beforeSend: function () {
...
function workflowStatusForm(id) {
    ...
    jQuery.ajax({
        url: getBaseURL() + 'advisor_task_workflows/add_workflow_status/' + id,
        type: "POST",
        dataType: "JSON",
        beforeSend: function () {
            jQuery("#loader-global").show();
...
function workflowStatusFormSubmit(container) {
    ...
    jQuery.ajax({
        url: getBaseURL() + 'advisor_task_workflows/add_workflow_status/',
        type: "POST",
        data: formData,
        dataType: "JSON",
        beforeSend: function () {
        ...
        success: function (response) {
            if (!response.result) {
                ...
                window.location = getBaseURL() + 'advisor_task_workflows/index/' + response.advisor_task_workflow_id;
            }
        }, complete: function () {
            jQuery('.modal-save-btn', container).removeAttr('disabled');
            jQuery('.loader-submit', container).removeClass('loading');
...
function statusTransitionsViewForm(statusId, workflowId) {
    jQuery.ajax({
        url: getBaseURL() + 'advisor_task_workflows/view_status_transitions/' + statusId + '/' + workflowId,
        dataType: 'JSON',
        type: 'POST',
        beforeSend: function () {
            jQuery("#loader-global").show();
...
function setAsStartPoint(workflowId, statusId) {
    jQuery.ajax({
        url: getBaseURL() + 'advisor_task_workflows/set_as_start_point/' + workflowId + '/' + statusId,
        dataType: 'JSON',
        type: 'POST',
        beforeSend: function () {
            jQuery("#loader-global").show();
...
function deleteWorkflowStatus(statusId) {
    var workflowId = jQuery('.ui-tabs-active', '#task-workflows-management').attr('aria-controls');
    jQuery.ajax({
        url: getBaseURL() + 'advisor_task_workflows/delete_workflow_status/' + statusId + '/' + workflowId,
        type: 'POST',
        success: function (response) {
...
function deleteTransition(transitionId) {
    ...
    jQuery.ajax({
        url: getBaseURL() + 'advisor_task_workflows/delete_transition/' + transitionId,
        dataType: 'JSON',
        type: 'POST',
        beforeSend: function () {
            jQuery("#loader-global").show();
        ...
        success: function (response) {
            if (response.result) {
                jQuery('#transition-' + transitionId, '#task-workflows-management').remove();
...
function deleteWorkflow(workflowId) {
    window.location = getBaseURL() + 'advisor_task_workflows/delete_workflow/' + workflowId;
}