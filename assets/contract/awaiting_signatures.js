var gridId = 'awaitingSignaturesGrid';
var awaitingSignaturesGrid = null;
var enableQuickSearch = false, customFieldsNames;
authIdLoggedIn = '';
var awaitingSignaturesSearchDataSrc, contractsSearchGridOptions;
jQuery(document).ready(function () {
    awaitingSignaturesGrid = jQuery('#' + gridId);
    try {
        if (awaitingSignaturesGrid.length > 0) {
            gridFiltersEvents('awaiting_signatures', 'awaitingSignaturesGrid', 'awaiting-signatures-filters');
            if(!gridHasDefaultFilter){
                jQuery('#signee_usersValue', '#awaiting-signatures-filters').val(signeeUser);
                jQuery('#signee_user_groupsValue', '#awaiting-signatures-filters').val(signeeUserGroup);
                jQuery('#requester_managerValue', '#awaiting-signatures-filters').val(signeeUser);
            }
            gridInitialization();
            jQuery('.k-pager-refresh-top', '.k-grid-info-refresh-top').on('click', function () {
                awaitingSignaturesGrid.data('kendoGrid').dataSource.read();
            });
            jQuery('.multi-select', '#awaiting-signatures-filters').chosen({no_results_text: _lang.no_results_matched, placeholder_text: _lang.select, width: "100%"}).change();

            jQuery('#awaiting-signatures-filters').bind('submit', function (e) {
                jQuery("form#awaiting-signatures-filters").validationEngine({
                    validationEventTrigger: "submit",
                    autoPositionUpdate: true,
                    promptPosition: 'bottomRight',
                    scroll: false
                });
                if (!jQuery('#awaiting-signatures-filters').validationEngine("validate")) {
                    return false;
                }
                e.preventDefault();
                ...
                awaitingSignaturesGrid.data('kendoGrid').dataSource.page(1);
            });
        }
    } catch (e) {
    }
...
function checkWhichTypeOfFilterIUseAndReturnFilters() {
    var filtersForm = jQuery('#awaiting-signatures-filters');
    disableEmpty(filtersForm);
    var searchFilters = form2js('awaiting-signatures-filters', '.', true);
    var filters = '';
    if (!enableQuickSearch) {
        filters = searchFilters.filter;
    } else if (jQuery('#quickSearchFilterValue', '#awaiting-signatures-filters').val()) {
...
function gridInitialization() {
    ...
    awaitingSignaturesSearchDataSrc = new kendo.data.DataSource({
        transport: {
            read: {
                dataType: "JSON",
                type: "POST",
                complete: function (XHRObj) {
                            ...
                            if (hasAccessToExport != 1) {
                                ...
                                if ($response.totalRows <= 10000) {
                                    if (jQuery(this).data('callexport').indexOf("true") >= 0) {
                                        exportAwaitingSignaturesToExcel(true);
                                    } else {
                                        exportAwaitingSignaturesToExcel();
                                    }
                                } else {
                    ...
                    animateDropdownMenuInGrids('awaitingSignaturesGrid');
                },
                beforeSend: function () {
            ...
            parameterMap: function (options, operation) {
                if ("read" == operation) {
                    ...
                    if (gridSavedFiltersParams) {
                        ...
                        setGridFiltersData(gridFormData, 'awaitingSignaturesGrid');
                    ...
                    options.sortData = JSON.stringify(awaitingSignaturesSearchDataSrc.sort());
    ...
    awaitingSignaturesSearchGridOptions = {
        autobind: true,
        dataSource: awaitingSignaturesSearchDataSrc,
    ...
    if (awaitingSignaturesGrid.length > 0) {
        gridTriggers({
            'gridContainer': awaitingSignaturesGrid,
            'gridOptions': awaitingSignaturesSearchGridOptions,
            'gridColumnsLength': Object.keys(tableColumns).length
        });
        var grid = awaitingSignaturesGrid.data('kendoGrid');
...
function loadEventsForFilters() {
    ...
    jQuery('#partyTypeOpertator', '#awaiting-signatures-filters').change(function () {
        jQuery('#partyNameValue', '#awaiting-signatures-filters').val('');
    });
    jQuery('#partyNameValue', '#awaiting-signatures-filters').autocomplete({
        autoFocus: true,
        delay: 600,
        source: function (request, response) {
            request.term = request.term.trim();
            var lookupType = jQuery('select#partyTypeOpertator', '#awaiting-signatures-filters').val();
            if (lookupType !== '') {
...
function exportAwaitingSignaturesToExcel(exportAllColumns) {
    ...
    jQuery('#filtersInfoForExport', newFormFilter).val(JSON.stringify(getExportInfoFilter('awaiting-signatures-filters')));
    ...
    newFormFilter.attr('action', getBaseURL('contract') + 'export/awaiting_signatures').submit();
}