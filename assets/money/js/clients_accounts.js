...
var gridDataSrc = new kendo.data.DataSource({
    transport: {
        read: {
            url: getBaseURL('money') + "accounts/clients",
            dataType: "JSON",
            type: "POST",
            complete: function () {
...
jQuery(document).ready(function () {
    ...
    gridOptions = {
        ...
        columns: [
            ...
            {field: 'account_number', template: '<a href="' + getBaseURL('money') + 'accounts/edit/#= id #">#= account_number #</a>', title: _lang.accountTypeAccountNumber.sprintf([_lang.client_Money]), width: '75px'},
            ...
            {field: 'description', title: _lang.description, template: '#= (description!=null&&description!="") ? ( description.length <= 70 ? description : description.substring(0,70)+"..." ) : ""#', width: '120px'}
        ],
        editable: "",
        filterable: false,
        height: 500,
        ...
        toolbar: [{
                ...
                template: '<div class="col-md-4 no-padding">'
                        + '<h4 class="col-md-5 no-padding-right">' + _lang.clientsAccounts + '</h4>'
                        ...
                        + '<a class="dropdown-item" href="' + getBaseURL('money') + 'accounts/suppliers" >' + _lang.listSupplierAccounts + ' </a>'
...
function exportAccountsToExcel() {
    ...
    newFormFilter.attr('action', getBaseURL('money') + 'accounts/export_clients_to_excel').submit();
}
