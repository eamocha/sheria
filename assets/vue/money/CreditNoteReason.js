    ...
    setup() {
        ...
        const initializeRecordObject = () => {
            if (recordEditIdGlobal > 0) {
                ...
                axios.get(Api.getApiBaseUrl("money") + '/credit_note_reasons/' + recordEditIdGlobal)
                    .then(response => {
                        if (response.data.credit_note_reason && response.data['credit_note_reason'].id) {
                            let recordResponse = response.data.credit_note_reason;
                            ...
                            setTimeout(() => window.location = getBaseURL('money') + 'credit_note_reasons/', 700);
                        }
                    }).catch((error) => {
        ...
        onMounted(() => {
            CommonInit.initialize(initializeRecordObject, getBaseURL('money') + 'credit_note_reasons/');
        });
        ...
        const saveRecord = () => {
            ...
            if (recordEditIdGlobal > 0) {
                axios.put(Api.getApiBaseUrl("money") + '/credit_note_reasons/' + reasonObj.value.id, reasonObj.value).then((response) => {
                    disableFields.value = false;
                    loader(false);
                ...
                axios.post(Api.getApiBaseUrl("money") + '/credit_note_reasons', Api.toFormData(reasonObj.value), Api.getInitialHeaders("multipart/form-data")).then((response) => {
                    if (response.data.credit_note_reason && response.data.credit_note_reason.id) {
                        pinesMessageV2({ ty: 'success', m: _lang.recordAddedSuccessfully });
                        setTimeout(() => window.location = getBaseURL('money') + 'credit_note_reasons/edit/' + response.data.credit_note_reason.id, 700);
                    } else {
                        disableFields.value = false;
        ...
        const deleteRecord = () => {
            if (!confirm(_lang.money.confirmationDeleteCreditNoteReason))
                return;
            loader(true);
            axios.delete(Api.getApiBaseUrl("money") + '/credit_note_reasons/' + reasonObj.value.id)
                .then((response) => {
