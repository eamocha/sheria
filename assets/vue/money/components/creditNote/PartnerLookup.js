        ...
        const getPartnersData = () => {
            ...
            timeout = setTimeout(() => {
                if (partnerName.value.length > 2) {
                    ...
                    axios.get(Api.getApiBaseUrl("money") + '/accounts?model_type=partner&organization_id=' + data.value.organization_id + '&name[ct]=' + encodeURIComponent(partnerName.value), Api.getInitialHeaders()).then(response => {
