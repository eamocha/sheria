...
export default {
    name: 'RelatedMatter',
    props: ['showmodal', 'matters', 'submitmatters', 'showdiscount', 'showtax', 'invoicedmatters', 'taxeslist'],
    ...
    setup(props, context) {
        ...
        const checkInvoicedMatters = () => {
            ...
            invoicedMatters.value.map((matter) =>{selectedMatters.value.push(matter.matter_id)})
        }
        const checkAllMatters = (flag) => {
            if (flag) matters.value.map((matter) => selectedMatters.value.push(matter.id))
            else selectedMatters.value = []
        }
        const checkAllTimeLogs = (flag) => {
            if (flag) timeLogs.value.map((timeLog) => selectedTimeLogs.value.push(timeLog.id))
            else selectedTimeLogs.value = []
        }
        const checkAllExpenses = (flag) => {
            if (flag) expenses.value.map((expense) => selectedExpenses.value.push(expense.expense_id))
        ...
        const mergeGroupBy = (array) => {
            ...
            for (const val in array) {
                ...
                array[val].map((time, index) => {
                    effectiveEffort = Number(effectiveEffort) + Number(time['effective_effort'])
        ...
        const submitModal = () => {
            props.submitmatters(selectedMattersDetails.value, selectedTimeLogsDetails.value, selectedExpensesDetails.value)
            resetModalCond()
        }
        const handleTimeLogDateFilter = (date) =>timeLogDateFilter.value = date

        const filterTimeLog = () => {
            timeLogDateFilter.value.operator = timeLogDateFilter.value.operator ?? "eq"
            timeLogDateFilter.value.from = timeLogDateFilter.value.from ?? moment().locale('en').toDate().format('Y-m-d')
            timeLogDateFilter.value.to = timeLogDateFilter.value.to ?? moment().locale('en').toDate().format('Y-m-d')
            timeLogs.value = mainTimeLogs.value.filter(time => {
                switch (timeLogDateFilter.value.operator) {
                    ...
                    case "gte":
                        return (time.log_date >= timeLogDateFilter.value.from);
                    case "btw":
                        return (time.log_date >= timeLogDateFilter.value.from && time.log_date <= timeLogDateFilter.value.to);
                    default:
                        return (time.log_date == timeLogDateFilter.value.from);
                }
            });
            ...
            if (selectedMatters.value.length == 0)
                ...
                switch (stepType) {
                    case "matters":
                        ...
                        timeLogs.value.sort((a, b) => new Date(a.log_date) - new Date(b.log_date));
                    ...
                    case "timeLogs":
                        selectedTimeLogsDetails.value = []
                        let userData = (timeLogUserData.value == "code") ? "user_code" : "user"
                        let timeArr = [];
                        timeLogs.value.map((timeLog) => {
                            if (selectedTimeLogs.value.includes(timeLog.id)) {
                                ...
                                timeLog.discount_percentage = selectedDiscount.value.discount_percentage
                                timeLog.user_data = timeLog[userData]
                                let timeLogCategory = langGlobal == "" ? timeLog['category'] :  timeLog[langGlobal+'_category'];
                                let extraInfo = '';
                                if(!timeLogCategory && !timeLog['comments'] ) 
                                    extraInfo = timeLog['log_date'] + " - " + timeLog['effective_effort'];
                                timeLog.description = ( timeLogCategory ? timeLogCategory : "") + extraInfo + ( timeLog['comments'] ? ((timeLogCategory) ? " - " : " ") + timeLog['comments'] : "" )
                        ...
                        selectedTimeLogsDetails.value = (timeLogUserGroup.value) ? groupTimeLogs(timeArr) : timeArr 
                        if(expenses.value.length > 0){
                            showSection.value = "expenses";
                            sectionName.value = _lang.relatedExpenses_Invoice;
                        }else{
    ...
    <div class="panel">
        <div class="p-fluid p-formgrid">
            <div v-if="matters.length==0">
                ${_lang.noRelatedMatters}
            </div>
            <div v-if="showSection=='matters'" class="col-md-12">
                <table class="table table-bordered"> 
                    <thead>
                        ...
                        <tr v-for="(singleMatter, index) in matters" :key="index">
                            ...
                            <td> 
                                <span class="badge badge-success mx-2" v-if="singleMatter['related_time_logs'].length > 0"> {{singleMatter['related_time_logs'].length}} ${labelsGlobal['time_logs']} </span>
                                <span  class="badge badge-success" v-if="singleMatter['related_expenses'].length > 0"> {{singleMatter['related_expenses'].length}} ${labelsGlobal['expenses']} </span>
                            </td>
                        </tr>
                    </tbody>
                </table>
                        ...
                        <table class="table table-bordered table-hover table-striped"> 
                            ...
                            <tbody>
                                <tr v-for="(singleTimeLog, index) in timeLogs" :key="index">
                                    ...
                                    <td>{{moneyLang == "" ? singleTimeLog.category : singleTimeLog[moneyLang+'_category'] }}</td>
                    ...
                    <div class="row">
                        <div class="col-sm-7">
                            <div class="col-sm-12">
                                ...
                                <h6 class="mt-4">${ _lang.helperInvoiceTimeLogs}</h6>
                                <div class="form-inline">
                            ...
                            <div class="col-sm-12 row mt-25">
                                <div v-if="showDiscount=='item_level' || showDiscount=='both_item_after_level' || showDiscount=='both_item_before_level'" class="col-sm-6 no-padding">
                                    ...
                                    <select class="form-control" id="discount" @change="onDiscountChange($event)" v-model="selectedDiscount.discount_id">
                                        ...
                                        <option v-for="(discount, i) in discounts" :key="i"  :value="discount.id">
                    ...
                    <table class="table table-bordered table-hover table-striped"> 
                        ...
                        <tbody>
                            <tr v-for="(singleExpense, index) in expenses" :key="index">
                                ...
                                <td>{{moneyLang == "" ? singleExpense.category : singleExpense[moneyLang+'_category'] }}</td>
