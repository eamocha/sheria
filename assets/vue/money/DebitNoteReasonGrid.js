...
export default {
    ...
    setup(props, context) {
        ...
        const title = langDebitNoteReason;
        const initErrorUrl = getBaseURL('money') + 'money_dashboards/';
        const searchListUrl = `${Api.getApiBaseUrl("money")}/debit_note_reasons`;
        const addRecordUrl = `${getBaseURL("money")}debit_note_reasons/add`;
        const responseResourceName = 'debit_note_reason';

        const columns = ref([
            { field: '', header: '', type: 'actions' },
            { field: 'name', header: langDebitNoteReason + ' (' + langSystemLanguage + ')', frozen: true, type: 'link', link: { path: 'modules/money/debit_note_reasons/edit/', key: 'id' } },
            { field: 'fl1name', header: langDebitNoteReason + ' (' + langSystemForeignLanguage_1 + ')' },
            { field: 'fl2name', header: langDebitNoteReason + ' (' + langSystemForeignLanguage_2 + ')' },
        ]);
        ...
        const optionsMenuItems = ref([
            {
                label: _lang.edit,
                icon: 'pi pi-pencil',
                command: () => { window.location = `${getBaseURL('money')}debit_note_reasons/edit/${selectedRecordId}`; }
            }, {
                label: _lang.delete,
                icon: 'pi pi-trash',
                command: () => { deleteDebitNoteReason(selectedRecordId); }
            }
        ]);

        function updateSelectedRecordId(id) { selectedRecordId = id; }

        function deleteDebitNoteReason(id) {
            commonGridRef.value.deleteRecord(`${Api.getApiBaseUrl("money")}/debit_note_reasons/${id}`);
        };
