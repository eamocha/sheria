...
function setCustomerPortalADUserSelectedCompany(company) {
    ...
    if (company.id && !jQuery('#customer-portal-user-company' + company.id, theWrapper).length) {
        theWrapper.append(jQuery('<div class="row multi-option-selected-items no-margin" id="customer-portal-user-company' + company.id + '"><span id="' + company.id + '">' + company.name + '</span> </div>').append(jQuery('<input type="hidden" value="' + company.id + '" name="companies_customer_portal_users[]" />')).append(jQuery('<input value="x" type="button" class="btn btn-default btn-xs float-right x-icon" onclick="jQuery(this.parentNode).remove();" />')));
    }
}
        ...
        success: function (response) {
            ...
            if (response.users) {
                ...
                for (i in response.users) {
                    ...
                    jQuery("#usersToAdd").append(jQuery('<div class="row multi-option-selected-items no-margin"><span>' + userEmail + '</span> </div>').append(jQuery('<textarea name="usersToAdd[]" class="d-none">' + userEncodedJSONData + '</textarea>')).append(jQuery('<input type="hidden" value="' + userEmail + '" class="users-to-add" />')).append(jQuery('<a onclick="unsetNewCaseMultiOption(this.parentNode);" tabindex="-1" class="btn btn-default btn-xs btn-link float-right" href="javascript:;"><i class="icon fa-solid fa-trash-can red"></i></a>')));
                }
            }
        },
