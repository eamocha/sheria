...
class Contract_types extends Contract_controller
{
    ...
    {
        $this->pageTitle = sprintf($this->lang->line("app4legal_browser_title"), $this->lang->line("contract_types"));
        $records = $this->contract_type_language->load_all_records();
        $data["applies_to"]=[];
        $data["records"] = [];
        ...
        $data["field_name"] = $this->lang->line("type");

        $data["field_type"] = "contract_types";
        $data["module"] = "contract";
        $this->load->view("partial/header");
        $this->load->view("contract_types/index", $data);
    ...
    public function add()
    {
        ...
        if ($this->input->post(NULL)) {
            ...
            $response = $this->contract_type_language->insert_record();
        }
    ...
    public function edit($id)
    {
        ...
        if ($this->input->get("add_edit_form")) {
            $data += $this->return_data();
            $data["records"] = $this->contract_type_language->load_record($id);
            $response["html"] = $this->load->view("administration/form", $data, true);
        }
        if ($this->input->post(NULL)) {
            $response = $this->contract_type_language->update_record($id);
        }
        $this->output->set_content_type("application/json")->set_output(json_encode($response));
    }
    ...
    public function delete()
    {
        ...
        if ($this->contract_type_language->delete_record($id)) {
