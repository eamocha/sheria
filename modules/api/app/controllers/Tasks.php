...
class Tasks extends Top_controller
{
    ...
    private function _load_data()
    {
        ...
        $data["taskTypes"] = $this->task_type->api_load_list_per_language($lang);
        $data["toMeId"] = $this->user_logged_in_data["user_id"];
        $data["toMeFullName"] = $this->user_logged_in_data["profileName"];
        $data["taskPriorities"] = array_combine($this->task->get("priorityValues"), [$this->lang->line("critical"), $this->lang->line("high"), $this->lang->line("medium"), $this->lang->line("low")]);
        $systemPreferences = $this->system_preferences["DefaultValues"];
        $data["default_values"] = ["task_type_id" => isset($systemPreferences["taskTypeId"]) && !empty($systemPreferences["taskTypeId"]) ? $systemPreferences["taskTypeId"] : "", "interval_date" => $this->system_preferences["Reminders"]["reminderIntervalDate"]];
        ...
        $data["assignments"] = $this->return_rule_per_type($data["default_values"]["task_type_id"]);
        return $data;
    }
    public function add()
    {
        ...
        if ($this->input->post(NULL)) {
            if (is_null($this->input->post("title"))) {
                ...
                $task_title = substr(strip_tags($this->input->post("description")), 0, $max_title_length);
                if (strlen($task_title) == $max_title_length) {
                    $task_title .= "...";
                }
                $this->task->set_field("title", $task_title);
            }
            $this->task->set_fields($this->input->post(NULL));
            $this->task->set_field("user_id", $this->user_logged_in_data["user_id"]);
            $this->task->set_field("archived", "no");
            $this->task->set_field("createdBy", $this->user_logged_in_data["user_id"]);
            $this->task->set_field("createdOn", date("Y-m-d H:i:s"));
            $this->task->set_field("modifiedOn", date("Y-m-d H:i:s"));
            $this->task->set_field("modifiedBy", $this->user_logged_in_data["user_id"]);
            ...
            $this->task->disable_builtin_logs();
            if ($this->input->post("estimated_effort")) {
                $estimated_effort = $this->timemask->humanReadableToHours($this->input->post("estimated_effort"));
                $this->task->set_field("estimated_effort", $estimated_effort);
            } else {
                $this->task->set_field("estimated_effort", 0);
            }
            if ($this->task->validate()) {
                ...
                if ($notify_before && $this->input->post("due_date") && (!isset($notify_before["time"]) || !$notify_before["time"] || !isset($notify_before["time_type"]) || !$notify_before["time_type"] || !isset($notify_before["type"]) || !$notify_before["type"] || ($is_not_nb = !is_numeric($notify_before["time"])))) {
                    ...
                    if ($this->input->post("assignment_id") && $this->input->post("user_relation") === $this->input->post("assigned_to")) {
                        ...
                        if ($this->assignment->get_field("assignment_rule") == "rr_algorithm") {
                            if ($this->input->post("legal_case_id") && $this->legal_case->fetch($this->input->post("legal_case_id"))) {
                                ...
                                $next_assignee = $this->assignment->load_next_task_assignee($this->input->post("assignment_id"), $this->provider_group->get_field("allUsers") != 1 ? $assigned_team : false);
                            } else {
                    ...
                    if ($this->task->insert()) {
                        ...
                        $caseId = $this->task->get_field("legal_case_id");
                        $this->notify_me_before_due_date($task_id, $caseId);
                        $response["success"]["msg"] = $this->lang->line("new_task_added_successfully");
                        $response["success"]["data"]["task_id"] = $task_id;
                        if ($caseId) {
                            if ($this->task->get_field("stage")) {
                                ...
                                $this->legal_case_litigation_detail->update_stage_order($this->task->get_field("stage"), $this->user_logged_in_data["user_id"]);
                            }
                        ...
                        $this->insert_related_users($task_id, "add_tasks");
                    } else {
                        $response["error"] = [];
                    }
                }
            } else {
                $response["error"] = $this->task->get("validationErrors");
            }
        } else {
    ...
    public function list_my_tasks()
    {
        $this->load_tasks(false);
    }
    public function list_all_tasks()
    {
        $this->load_tasks(true, $this->input->post("user_id"));
    }
    private function load_tasks($all_tasks = true, $tasks_by_user_id = 0, $contributed = false)
    {
        $response = $this->responseData;
        ...
        $response["success"] = $this->task->api_load_all_tasks($user_id, $overridePrivacy, $authLang, $pageSize, $skip, $term, false, $this->input->post("filters"), $all_tasks, $tasks_by_user_id, $contributed, $contributors_ids);
        if (isset($response["success"]["data"]) && !empty($response["success"]["data"])) {
            foreach ($response["success"]["data"] as $key => $value) {
                ...
                $response["success"]["data"][$key]["effectiveEffort"] = $response["success"]["data"][$key]["effectiveEffort"] == ".00" ? "" : $this->timemask->timeToHumanReadable($response["success"]["data"][$key]["effectiveEffort"]);
    ...
    public function list_tasks_reported_by_me()
    {
        $response = $this->responseData;
        $user_id = $this->user_logged_in_data["user_id"];
        $this->user_profile->fetch(["user_id" => $user_id]);
        ...
        $term = trim((string) $this->input->post("term"));
        $_POST["filters"] = $this->input->post("filters") ? $this->input->post("filters") : [];
        $response["success"] = $this->task->api_load_all_tasks($user_id, $overridePrivacy, $authLang, $pageSize, $skip, $term, true, $this->input->post("filters"), false, NULL, false, NULL);
        if (isset($response["success"]["data"]) && !empty($response["success"]["data"])) {
            foreach ($response["success"]["data"] as $key => $value) {
                $response["success"]["data"][$key]["estimated_effort"] = $response["success"]["data"][$key]["estimated_effort"] == ".00" ? "" : $this->timemask->timeToHumanReadable($response["success"]["data"][$key]["estimated_effort"]);
                $response["success"]["data"][$key]["effectiveEffort"] = $response["success"]["data"][$key]["effectiveEffort"] == ".00" ? "" : $this->timemask->timeToHumanReadable($response["success"]["data"][$key]["effectiveEffort"]);
    ...
    public function autocomplete()
    {
        ...
        if (!empty($term)) {
            ...
            $this->user_profile->fetch(["user_id" => $user_id]);
            $overridePrivacy = $this->user_profile->get_field("overridePrivacy");
            $response["success"]["data"] = $this->task->api_lookup($term, $user_id, $overridePrivacy, $this->input->post("legal_case_id"));
    ...
    public function edit($task_id = 0)
    {
        ...
        if (0 < $task_id) {
            ...
            $overridePrivacy = $this->user_profile->get_field("overridePrivacy");
            $data = $this->task->api_load_task($task_id, $loggedUserId, $overridePrivacy);
            if ($data) {
                $data["contributors"] = $this->task->load_task_contributors($task_id);
                $data["estimated_effort"] = $data["estimated_effort"] == ".00" ? "" : $this->timemask->timeToHumanReadable($data["estimated_effort"]);
                $data["notify_before"] = $this->reminder->load_notify_before_data_to_related_object($task_id, $this->task->get("_table"), $loggedUserId);
                ...
                $transitions_accessible = $this->task_workflow_status_transition->load_available_steps($data["task_status_id"], $data["workflow"], $this->user_logged_in_data["user_id"], $this->user_logged_in_data["user_group_id"]);
                ...
                if (0 < $data["task_status_id"]) {
                    $this->load->model("task_status");
                    $this->task_status->fetch($data["task_status_id"]);
                    $data["workflowStatusByName"] = $this->task_status->get_field("name");
                }
                $response["success"]["data"]["taskValues"] = $data;
                $reporter_user = (int) $response["success"]["data"]["taskValues"]["reporter"];
                $response["success"]["data"]["restricted_by_task_reporter"] = $only_reporter_edit_meta_data == "1" && $loggedUserId != $reporter_user ? true : false;
                $response["success"]["data"]["disables_fields"] = $response["success"]["data"]["restricted_by_task_reporter"] ? ["title", "description", "assigned_to", "reporter", "due_date", "task_type_id"] : "";
                $lang = $this->get_lang_code();
                $this->load->model("legal_case_litigation_detail", "legal_case_litigation_detailfactory");
                $this->legal_case_litigation_detail = $this->legal_case_litigation_detailfactory->get_instance();
                ...
                $response["error"] = $this->lang->line("task_id_not_exists");
            }
        } else {
            if ($this->input->post(NULL)) {
                if (!$this->input->post("id") || !$this->input->post("id")) {
                    $response["error"] = "missing task id";
                } else {
                    if ($this->task->fetch($this->input->post("id"))) {
                        $reporter_user = (int) $this->task->get_field("reporter");
                        $restricted_by_task_reporter = $only_reporter_edit_meta_data == "1" && $loggedUserId != $reporter_user ? true : false;
                        $old_status = $this->task->get_field("task_status_id");
                        $old_type = $this->task->get_field("task_type_id");
                        ...
                        if ($restricted_by_task_reporter) {
                            ...
                            if (isset($post_data["task_type_id"])) {
                                unset($post_data["task_type_id"]);
                            }
                        }
                        $this->task->set_fields($post_data);
                        $this->task->set_field("task_status_id", $old_status);
                        if ($this->input->post("task_type_id") !== $old_type) {
                            $this->load->model("task_workflow", "task_workflowfactory");
                            $this->task_workflow = $this->task_workflowfactory->get_instance();
                            $workflow_applicable = $this->task_workflow->load_workflow_task_status_per_type($this->input->post("task_type_id")) ?: $this->task_workflow->load_default_system_workflow();
                            $this->task->set_field("task_status_id", $workflow_applicable["status"]);
                        ...
                        if ($this->task->validate()) {
                            $notify_before = $this->input->post("notify_me_before");
                            if ($notify_before && ($this->input->post("due_date") || $this->task->get_field("due_date")) && (!isset($notify_before["time"]) || !$notify_before["time"] || !isset($notify_before["time_type"]) || !$notify_before["time_type"] || !isset($notify_before["type"]) || !$notify_before["type"])) {
                                $response["error"] = [];
                                $response["error"]["notify_before"] = $this->lang->line("cannot_be_blank_rule");
                            } else {
                                if ($this->task->update()) {
                                    ...
                                    $task_id = $this->task->get_field("id");
                                    $this->notify_me_before_due_date($task_id, $this->task->get_field("legal_case_id"));
                                    $this->insert_related_users($task_id, "edit_task");
                                    $response["success"]["msg"] = sprintf($this->lang->line("record_save_successfull"), " " . $this->task->get("modelCode") . $task_id);
                                } else {
                                    $response["error"] = [];
                                }
                            }
    ...
    private function notify_me_before_due_date($task_id, $case_id)
    {
        $notify_before = $this->input->post("notify_me_before");
        $due_date = $this->input->post("due_date") ? $this->input->post("due_date") : $this->task->get_field("due_date");
        if (empty($this->reminder)) {
            $this->load->model("reminder", "reminderfactory");
            $this->reminder = $this->reminderfactory->get_instance();
        }
        $current_reminder = $this->reminder->load_notify_before_data_to_related_object($task_id, $this->task->get("_table"), $this->user_logged_in_data["user_id"]);
        ...
        if ($notify_before && $due_date) {
            $reminder = ["user_id" => $this->user_logged_in_data["user_id"], "remindDate" => $due_date, "legal_case_id" => $case_id, "task_id" => $task_id, "related_id" => $task_id, "related_object" => $this->task->get("_table"), "notify_before_time" => $notify_before["time"], "notify_before_time_type" => $notify_before["time_type"], "notify_before_type" => $notify_before["type"]];
            $reminder["summary"] = sprintf($this->lang->line("notify_me_before_message"), $this->lang->line("task"), $this->task->get("modelCode") . $task_id, $due_date);
            return $this->reminder->remind_before_due_date($reminder, isset($notify_before["id"]) ? $notify_before["id"] : NULL);
        }
        return false;
    }
    private function insert_related_users($task_id, $action)
    {
        ...
        $watchers = $this->input->post("Task_Users", true) ?? [];
        if ($contributors) {
        ...
        $watchers_data["users"] = ["task_id" => $task_id, "users" => $watchers];
        $this->task->insert_task_users($watchers_data);
    ...
    public function view($id)
    {
        ...
        $task_data = $this->task->api_load_task($id, $logged_user, $override_privacy);
        $data["task_data"] = $task_data;
        $data["task_data"]["estimated_effort"] = $this->timemask->timeToHumanReadable($data["task_data"]["estimated_effort"]);
        $data["task_data"]["effectiveEffort"] = $this->timemask->timeToHumanReadable($data["task_data"]["effectiveEffort"]);
        $data["task_data"]["model_code"] = $this->task->get("modelCode");
        $this->load->model("task_workflow_status_transition", "task_workflow_status_transitionfactory");
        $this->task_workflow_status_transition = $this->task_workflow_status_transitionfactory->get_instance();
        $transitions_accessible = $this->task_workflow_status_transition->load_available_steps($data["task_data"]["task_status_id"], $data["task_data"]["workflow"], $this->user_logged_in_data["user_id"], $this->user_logged_in_data["user_group_id"]);
        $data["available_statuses"] = $transitions_accessible["available_statuses"];
        $data["status_transitions"] = $transitions_accessible["status_transitions"];
        $this->load->model("custom_field", "custom_fieldfactory");
        $this->custom_field = $this->custom_fieldfactory->get_instance();
        $custom_fields = $this->custom_field->load_custom_fields($id, $this->task->get("modelName"), $this->get_lang_code());
        ...
        if ($task_data["private"] === "yes") {
            $data["watchers"] = $this->task->load_task_users($id);
        }
        $data["contributors"] = $this->task->load_task_contributors($id);
        $response["success"]["data"] = $data;
        $this->render($response);
    }
    public function move_status($task_id = "", $status_id = "")
    {
        $this->check_license_availability();
        $response = $this->responseData;
        if (!$task_id || !$status_id) {
            ...
            $this->load->model("task_workflow_status_transition", "task_workflow_status_transitionfactory");
            $this->task_workflow_status_transition = $this->task_workflow_status_transitionfactory->get_instance();
            if (!$this->task_workflow_status_transition->check_transition_allowed($task_id, $status_id, $this->user_logged_in_data["user_id"], $this->user_logged_in_data["user_group_id"])) {
                ...
                $this->task_fields->load_all_fields($lang);
                $this->task->fetch($task_id);
                $old_status = $this->task->get_field("task_status_id");
                $workflow_applicable = 0 < $this->task->get_field("workflow") ? $this->task->get_field("workflow") : 1;
                $this->task_workflow_status_transition->fetch(["workflow_id" => $workflow_applicable, "from_step" => $old_status, "to_step" => $status_id]);
                $transition = $this->task_workflow_status_transition->get_field("id");
                $data = $this->task_fields->return_screen_fields($task_id, $transition, $lang);
                if ($data) {
    ...
    public function add_screen_transition()
    {
        ...
        if (!$task_id || !$transition || !$this->legal_case->fetch($task_id)) {
            $response["error"] = "missing data";
        } else {
            $this->load->model("task_fields", "task_fieldsfactory");
            $this->task_fields = $this->task_fieldsfactory->get_instance();
            ...
            $this->load->model("task_workflow_status_transition", "task_workflow_status_transitionfactory");
            $this->task_workflow_status_transition = $this->task_workflow_status_transitionfactory->get_instance();
            $this->task_workflow_status_transition->fetch($transition);
            $status = $this->task_workflow_status_transition->get_field("to_step");
            $validation = $this->task_fields->validate_fields($transition);
            if (!$validation["result"]) {
                $response["error"] = $validation["errors"];
            } else {
                $this->task->set_field("task_status_id", $status);
                if (!$this->task->update()) {
                    $response["error"] = $this->lang->line("workflowActionInvalid");
                } else {
                    $save_result = $this->task_fields->save_fields($task_id, $this->user_logged_in_data["user_id"]);
    ...
    public function add_comment()
    {
        ...
        if ($this->input->post(NULL)) {
            ...
            $this->task_comment->set_field("createdBy", $this->user_logged_in_data["user_id"]);
            $this->task_comment->set_field("createdOn", date("Y-m-d H:i:s"));
            $this->task_comment->set_field("modifiedBy", $this->user_logged_in_data["user_id"]);
            $this->task_comment->set_field("modifiedOn", date("Y-m-d H:i:s"));
            $this->task_comment->disable_builtin_logs();
            if ($this->task_comment->insert()) {
                $response["success"]["msg"] = $this->lang->line("comment_added_successfully");
            } else {
                $response["error"] = $this->task_comment->get("validationErrors");
            }
        } else {
            $response["error"] = $this->lang->line("data_missing");
        }
    ...
    public function edit_comment($comment_id = "", $task_id = "")
    {
        $response = $this->responseData;
        $this->load->helper("format_comment_patterns");
        ...
        if ($comment_id && $task_id) {
            $this->task_comment->fetch(["id" => $comment_id]);
            $response["success"]["data"] = $this->task_comment->get_fields();
            ...
            if ($this->input->post(NULL)) {
                ...
                $task_id = $this->input->post("task_id");
                $this->task_comment->fetch(["id" => $comment_id, "task_id" => $task_id]);
                $_POST["comment"] = format_comment_patterns($this->regenerate_note($this->input->post("comment")), true, true);
                if ($this->input->post("comment") != $this->task_comment->get_field("comment")) {
                    $_POST["edited"] = 1;
                }
                $this->task_comment->set_fields($this->input->post(NULL));
                $this->task_comment->set_field("modifiedBy", $this->user_logged_in_data["user_id"]);
    ...
    public function comments()
    {
        ...
        $this->task_comment = $this->task_commentfactory->get_instance();
        $this->load->helper("revert_comment_html");
        $comments = $this->task_comment->load_comments($id, $this->input->get("showAll"));
        if (0 < $comments["count"]) {
    ...
    public function upload_file()
    {
        ...
        foreach ($_FILES as $file_key => $file) {
            if (!empty($file["name"])) {
                $upload_response = $this->dms->upload_file(["module" => "task", "module_record_id" => $this->input->post("task_id"), "upload_key" => $file_key]);
    ...
    public function download_file($file_id)
    {
        if ($file_id) {
            ...
            $response = $this->dms->get_file_download_data("task", $file_id);
    ...
    public function return_doc_thumbnail($id = 0)
    {
        if ($id) {
            ...
            $image_dir = $documents_root_direcotry . "tasks" . $this->document_management_system->get_field("lineage");
        ...
        if (isset($matches_link) && !empty($matches_link)) {
            foreach ($matches_link[0] as $key => $value) {
                ...
                if (isset($file_name_link[$key][1]) && is_numeric(str_replace("]", "", $file_name_link[$key][1]))) {
                    ...
                    $attachments_links[$key]["download_link"] = base_url() . "tasks/download_file/" . str_replace("]", "", $file_name_link[$key][1]);
        ...
        if (isset($images_tag) && !empty($images_tag)) {
            foreach ($images_tag[0] as $k => $val) {
                ...
                if (isset($file_name_image[$k][1]) && is_numeric(str_replace("!", "", $file_name_image[$k][1]))) {
                    ...
                    $attachments_images[$k]["download_link"] = base_url() . "tasks/return_doc_thumbnail/" . str_replace("!", "", $file_name_image[$k][1]);
    ...
    private function return_rule_per_type($type)
    {
        ...
        $this->assignment = $this->assignmentfactory->get_instance();
        $data = [];
        if ($type && $this->assignment->fetch(["category" => "task", "type" => $type])) {
        ...
        if ($data) {
            switch ($data["assignment_rule"]) {
                case "rr_algorithm":
                    $next_assignee = $this->assignment->load_next_task_assignee($data["id"]);
    ...
    private function send_notifications($object, $users = [])
    {
        ...
        $task_id = $this->task->get_field("id");
        $assignee = $this->task->get_field("assigned_to");
        $assignee_id = str_pad($this->task->get_field("assigned_to"), 10, "0", STR_PAD_LEFT);
        $reporter_id = str_pad($this->task->get_field("reporter"), 10, "0", STR_PAD_LEFT);
        ...
        $this->load->model("task_status");
        $this->task_status->fetch($this->task->get_field("task_status_id"));
        $task_status = $this->task_status->get_field("name");
        ...
        $legal_case_subject = "";
        if ($this->task->get_field("legal_case_id") != 0) {
            $this->legal_case->fetch(["id" => $this->task->get_field("legal_case_id")]);
            $subject = $this->legal_case->get_field("subject");
            $legal_case_subject = $this->legal_case->get("modelCode") . $this->task->get_field("legal_case_id") . ": " . (42 < strlen($subject) ? mb_substr($subject, 0, 42) . "..." : $subject);
        }
        $notificationsData = ["toIds" => array_unique($toIds), "object" => $object, "object_id" => $task_id, "objectModelCode" => $this->task->get("modelCode"), "targetUser" => $assignee, "taskData" => ["taskID" => $task_id, "taskType" => $taskType, "taskStatus" => $task_status, "priority" => $this->task->get_field("priority"), "dueDate" => $this->task->get_field("due_date"), "title" => $this->task->get_field("title"), "taskDescription" => nl2br($this->task->get_field("description")), "modifiedBy" => $this->email_notification_scheme->get_user_full_name($this->task->get_field("modifiedBy")), "assignee" => $assignee_profile_name, "legal_case" => $legal_case_subject, "legal_case_id" => $this->task->get_field("legal_case_id")], "attachments" => []];
