...
class Migration_Create_Legal_Cases_Grid_View extends CI_Migration {

    public function up()
    {
        $this->db->trans_start(); // Start a transaction for atomicity

        // SQL to drop the view if it already exists
        $drop_view_sql = "
            IF OBJECT_ID('[dbo].[legal_cases_grid]', 'V') IS NOT NULL
                DROP VIEW [dbo].[legal_cases_grid];
        ";
        $this->db->query($drop_view_sql);

        // SQL to create the legal_cases_grid view
        $create_view_sql = "
        ...
        ";
        // Splitting by GO and executing is not needed here as it's a single CREATE VIEW statement
