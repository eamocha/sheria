        ...
        <div class="col-md-12">
            <div class="col-md-8 no-padding">
                <span id="title-description"><h4 class="opinion-subject"></bdi>
                        <?php echo $opinion_data["model_code"];
        ...
        <div class="col-md-12">
            ...
            foreach ($first_statuses as $status_id => $status_name) {
                ...
                if (isset($step["id"]) && $step["id"]) {?>
                    <a href="javascript:;" onclick="screenTransitionForm('<?php  echo $opinion_data["id"];?>', '<?php   echo $step["id"];?>', 'legal_opinions');" title=" <?php         echo isset($step["comments"]) && $step["comments"] !== "" ? $step["comments"] : $step_name; ?>" class="case-move-status-link btn btn-default">
            ...
            <div class="caseStatusesVisibleList">
                <div class="dropdown"><?php if (!empty($other_statuses)) {?>
                        ...
                        foreach ($other_statuses as $status_id => $status_name) {
                            ...
                            $step_name = $step["name"] ? $step["name"] : $status_name;
                            if (isset($step["id"]) && $step["id"]) {?>
                                <a class="dropdown-item" href="javascript:;" onclick="screenTransitionForm('<?php echo $opinion_data["id"];?>', '<?php echo $step["id"];?>', 'legal_opinions');" title=" <?php  echo isset($step["comments"]) && $step["comments"] !== "" ? $step["comments"] : $step_name;?>"><?php  echo $step_name;?></a><?php  echo $step_name; ?></a>
                            <?php   }
                            else {?>
            ...
            <div class="d-flex mt-20" id="body-content">
                <div class="col-md-8 col-xs-8" id="main-section">
                    ...
                    <?php if (isset($opinion_data["caseSubject"]) && $opinion_data["caseSubject"]) {?>
                        <div id="matter-attachments-module" class="module"><?php $this->load->view("opinions/view/matter_attachments_section");?> </div>          <?php }?>

                    <div id="activity-module" class="module"><?php $this->load->view("opinions/view/activity_section");?></div>
                    <button type="button" class="btn btn-default navbar-btn" onclick="opinionCommentFormInline('<?php echo $opinion_data["id"];?>');"><?php echo $this->lang->line("comment");?>&nbsp;&nbsp;<i class="fa-solid fa-comment"></i></button>
                </div>
                <div class="px-3" id="side-section"><?php $this->load->view("opinions/view/side_section");?> </div>
            </div>
        </div>
        <div id="attachments-module2" class="module"><?php $this->load->view("opinions/view/attachments");?></div>
        <script>
