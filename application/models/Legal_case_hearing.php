...
class mysql_Legal_case_hearing extends My_Model
{
    ...
    {
        $this->_table = "legal_case_hearings_full_details AS legal_case_hearings"; // Alias the table
        ...
        $query["select"] = ["legal_case_hearings.id,legal_case_hearings.comments,legal_case_hearings.hearingID,legal_case_hearings.legal_case_id,legal_case_hearings.createdOn, legal_case_hearings.startDate, legal_case_hearings.startTime, legal_case_hearings.postponedDate, legal_case_hearings.postponedTime, types.name as type_name"];
        // Add join condition
        $query["join"] = [["hearing_types_languages as types", "types.type = legal_case_hearings.type and types.language_id = '" . $langId . "'", "left"]];
...
class sqlsrv_Legal_case_hearing extends mysql_Legal_case_hearing
{
    ...
    public function get_legalCase_task_summary($legal_case_id) {
        // Specify the table and alias.
        $this->_table = 'tasks_detailed_view AS tasks_view';
        $this->ci->load->model("language");
        $langId = $this->ci->language->get_id_by_session_lang();
        // Define the SELECT query.  Use proper syntax for the array.
        $query['select'] = ["tasks_view.id ,tasks_view.task_type_id ,tasks_view.title ,tasks_view.taskFullDescription ,tasks_view.assigned_to ,tasks_view.createdBy ,tasks_view.taskStatus ,tasks_view.due_date,ttl.name as taskType" ];
       // $query["join"] = [["hearing_types_languages as types", "types.type = legal_case_hearings.type and types.language_id = '" . $langId . "'", "left"]];
//task_types_languages ttl
        $query["join"] = [["task_types_languages as ttl", "ttl.task_type_id = tasks_view.task_type_id and ttl.language_id = '" . $langId . "'", "left"]];
        $query['where'] = [
            ['legal_case_id' , $legal_case_id] // Corrected where clause format
        ];
