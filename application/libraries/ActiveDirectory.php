...
class ActiveDirectory
{
    ...
    public function __construct()
    {
        ...
        $this->host = $systemPreferences["host"];
        $this->adminUser = $systemPreferences["username"];
        $this->adminPassword = $this->ci->encryption->decrypt($systemPreferences["password"]);
        $this->domain = $systemPreferences["domain"];
        $this->port = $systemPreferences["port"];
        $this->mail_option = $systemPreferences["emailMappingOption"];
        $this->base_dn = "DC=ca,DC=go,DC=ke";
        $connection = $this->connect();
        if ($connection === "ldap_disabled") {
            ...
            if ($this->connect() === false) {
                ...
                $this->write_log("Could not connect to Active Directory Server", ldap_error($this->connection), "ERROR");
...

    {
        ...
        if (!$this->connection) {
            ...
            if (function_exists("ldap_connect")) {
                ...
                ldap_set_option($connection, LDAP_OPT_PROTOCOL_VERSION, 3) or exit("Unable to set LDAP protocol version");
                ...
                if ($this->connection) {
                    ...
                    $this->write_log("connection Ok " );

                    return true;

                }

                $this->write_log("connection false " );

                return false;

            }

            $this->write_log("connection disabled " );
...

    {
        ...
        if (!$connection) {
            ...
            $this->write_log("Connection failed", "error");
        ...
        $this->write_log("Connecting to AD server using Admin account", "info");


        $bind = @ldap_bind($connection, $this->adminUser, $this->adminPassword);
        ...
        if ($bind) {
            ...
            $this->write_log("Successfully bound to AD", "info");

            return true;

        } else {

            $this->write_log("Authentication Failed | " . ldap_error($connection), "error");
...

    {
        ...
        $bind = @ldap_bind($connection, $this->admin_username, $this->admin_password);
...

    {
        ...
        unset($entries["count"]); // clean up before passing to fix_returned_data
