...
class Cases extends Core_controller
{
    public $Legal_Case;
...
/// end of exihibt
    ...
    public function edit($id = 0,$view="edit")
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $legalCasewatchersUsers = $this->legal_case->load_watchers_users($id);
            $isLitigationCase = $legalCase["category"] == "Litigation" ??"criminal"??false;
            ...
            $this->includes("scripts/matter_fee_notes","js");
              $this->includes("scripts/case_docs_summary","js");
            $this->includes("scripts/case_opponents","js");
            ...
            $this->load->view("partial/header");
            $this->load->view("cases/".$view.$view_file, $data);
        }
    }
...
///remove_party
    ...
    public function upload_file_from_case_view($case_id)
    {
        ...
        $data["lineage"] = "";
        $data["type"] = "";
        $data["module_record_id"] = $case_id;
        $response["message"]="Successfully Saved";
        $response["success"]=true;
        $response["html"] = $this->load->view("cases/main_page_docs/uploadform", $data, true);
    ...
    public function upload_file()
    {
        ...
        if ($this->input->post("dragAndDrop")) {
            ...
            $html = "<html>  <head>    <meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\" /><script type=\"text/javascript\">\r\n                        if(window.top.uploadDocumentDone) window.top.uploadDocumentDone('" . $response["message"] . "', '" . ($response["status"] ? "success" : "error") . "');</script>              </head>        </html>";
...
//        $caseId = $this->legal_case->get_field("id");
        ...
        if (!is_array($opponent_member_types)) {
            $opponent_member_types = $opponent_member_types ? [$opponent_member_types] : [];
        }
        if (!is_array($opponent_member_ids)) {
            $opponent_member_ids = $opponent_member_ids ? [$opponent_member_ids] : [];
        }
        // Safeguard: use the count of the smaller array to prevent errors if they are mismatched
        $count = min(count($opponent_member_types), count($opponent_member_ids));
...
//        $this->load->model("legal_case_company");
        ...
        // If opponent_member_types is NOT an array, wrap it if it has content, otherwise use empty array.
        if (!is_array($opponent_member_types)) {
            $opponent_member_types = $opponent_member_types ? [$opponent_member_types] : [];
        }
        // Repeat for IDs
        ...
        // Use the count of the smallest array for safe iteration
        $count = min(count($opponent_member_types), count($opponent_member_ids));
        $caseId = $this->legal_case->get_field("id");
        $this->load->model("legal_case_company");
        for ($opp = 0; $opp < count($opponent_member_types); $opp++) {
    ...
    public function add_legal_matter($container_id = 0)
    {
        ...
        if ($this->request_can_cause_insufficient_anti_automation("Corporate_Matter")) {
            ...
            if (!$this->input->post(NULL)) {
                ...
                $data = $formData;
                $data["systemPreferences"] = $system_preferences;
                $data["defaultCompanyId"]=$system_preferences['defaultCompany']??1;
                ...
                if ($this->legal_case->validate() && !$lookup_validate) {
                    ...
                    if ($notify_before && $this->input->post("dueDate") && (!$notify_before["time"] || !$notify_before["time_type"] || !$notify_before["type"] || ($is_not_nb = !is_numeric($notify_before["time"])))) {
                        ...
                        if ($this->legal_case->insert()) {
                            ...
                            $this->criminal_case_detail->set_field("origin_of_case", $post_data["approval_step"]==1?"Public Complaint/Inquiry":"Surveilance Detection");
...
//function to add attachments
    ...
    public function load_quick_add_feeNote_form($case_id=0,$ext_counsel_id=0)
    {
        ...
        $this->load->model("client");
        $clientData=$this->client->load_case_client_details($case_id);
        //$system_preferences =  $this->session->userdata("systemPreferences");
        $data["client_id"]=$clientData["clientId"];//$system_preferences['defaultCompany'];
        $data["case_id"]=$case_id;
        $data["supplier_id"]=$ext_counsel_id;


        $data["account"]=39;


        $response = ["success" => true, "message" => "Fee note form loaded successfully"];
